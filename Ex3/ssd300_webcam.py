...
while True:
    ...
    ret, img = cam.read()
    ori_height, ori_width, channel= img.shape #Save the size of original frame
    img_re = imresize(img,(img_height, img_width), preserve_range=True)
    img_re = np.array([img_re])

    # Predict the object
    prediction = model.predict(img_re)
    # Extract valid data
    prediction = prediction[(prediction > 0).any(axis=2)]

    height_ratio = int(ori_height/img_height)
    width_ratio = int(ori_width/img_width)

    color = (255, 0, 0)
    thickness = 2
    X, Y = prediction.shape
    # Make a rectangle for found objects
    for y in range(X):

        cv2.rectangle(img
                      , (int(prediction[y, 2] * width_ratio), int(prediction[y, 3] * height_ratio))
                      , (int(prediction[y, 4] * width_ratio), int(prediction[y, 5] * height_ratio))
                      , color
                      , thickness
                      )
        cv2.putText(img
                      , classes[int(prediction[y, 0])]
                      , (int(prediction[y, 2]*width_ratio), int(prediction[y, 3]*height_ratio-10))
    ...
    #prediction.xmin*width_ratio, prediction.ymin*height_ratio, prediction.xmax*width_ratio, prediction.ymax*height_ratio

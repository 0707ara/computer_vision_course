...
while cv2.waitKey(delay) != ord('q'):
    try:
        ...
        if len(detections) != 0:
            ...
            for x in range(X):
                cv2.rectangle(img
                              , (int(detections[x, 0]), int(detections[x, 1]))
                              , (int(detections[x, 2]), int(detections[x, 3]))
                              , color
                              , thickness
                              )

        #cv2.rectangle(img, (detections.x, detections.y), (detections.x + detections.w, detections.y + detections.h), (0, 0, 255), 2)
        ##--your-code-ends-here--##
